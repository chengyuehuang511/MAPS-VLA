...
def layer_key_from_param_name(name: str) -> str:
    """
    Map a param name to a 'layer key' for aggregation.
    Example keys:
      - vision_backbone.featurizer.blocks.7
      - language_model.model.layers.12
      - projector.fc1
    Strategy: take prefix up to first numeric token; otherwise take first 2-3 tokens.
    """
    toks = name.split(".")
    for i, t in enumerate(toks):
        if t.isdigit():
            return ".".join(toks[: i + 1])  # include the index
    # no numeric segment -> coarser grouping
    return ".".join(toks[:3]) if len(toks) >= 3 else name

@torch.no_grad()
def compute_layerwise_rms_grad_norm(model: nn.Module) -> "OrderedDict[str, float]":
    """
    Returns OrderedDict[layer_key] = RMS grad norm over params in that layer_key.
    RMS = sqrt( mean(g^2) ) across all elements in the layer group.
    """
    sumsq_by_layer = defaultdict(float)
    count_by_layer = defaultdict(int)

    for name, p in model.named_parameters():
        if (p.grad is None) or (not p.requires_grad):
            continue
        key = layer_key_from_param_name(name)
        g = p.grad.detach()
        sumsq_by_layer[key] += g.pow(2).sum().item()
        count_by_layer[key] += g.numel()

    # stable ordering: sort keys alphabetically (or customize)
    keys_sorted = sumsq_by_layer.keys()

    out = OrderedDict()
    for k in keys_sorted:
        denom = max(count_by_layer[k], 1)
        out[k] = math.sqrt(sumsq_by_layer[k] / denom)
    return out

@torch.no_grad()
def compute_layerwise_grad_norm(model: nn.Module) -> "OrderedDict[str, float]":
    """
    Per-layer L2 gradient norm:
        ||g_layer||_2 = sqrt(sum_i g_i^2)

    This is the *raw* gradient norm (NOT size-normalized).
    """
    sumsq_by_layer = defaultdict(float)
    ...
    out = OrderedDict()
    for k in sumsq_by_layer.keys():
        out[k] = math.sqrt(sumsq_by_layer[k])
...
class FinetuneConfig:
    ...
    use_prismatic_vlm: bool = False                  # Whether to use Prismatic VLM architecture
    ...
    use_vq_action_tokenizer: bool = False            # If True, uses VQ-based action tokenizer instead of uniform binning
    ...
    unfreeze_early_language: bool = False           # If True, unfreeze the early layers of the language model.
    ...
    unfreeze_dino: bool = False                      # If True, unfreeze all parameters of the dino backbone.
...
def get_run_id(cfg) -> str:
    ...
    # elif cfg.resume and not cfg.use_lora:
    ...
    #         run_id = "--".join(run_id.split("--")[:-1])
        ...
        if cfg.use_vq_action_tokenizer:
            run_id += f"+vq_action_tokenizer"
        if cfg.use_fz:
        ...
        if cfg.freeze_vlm:
            ...
            if cfg.unfreeze_early_language:
                run_id += f"+unfreeze_early_language"
        ...
        if cfg.unfreeze_dino:
            run_id += f"+unfreeze_dino"
        ...
        if cfg.weight_decay_scheduler == "layerwise_decay_cosine":
            run_id += f"+layerwise_decay_cosine"
        if cfg.image_aug:
            run_id += "--image_aug"
...
def run_forward_pass(
    ...
    if not (use_l1_regression):
        ...
        if cfg.use_vq_action_tokenizer:
            current_action_mask = (action_tokenizer.action_token_end_idx > ground_truth_token_ids) & (ground_truth_token_ids > action_tokenizer.action_token_begin_idx)
            next_actions_mask = current_action_mask
    ...
    # Compute metrics for continuous action representations (L1 regression)
        ...
        # Get hidden states for action portion of response
        batch_size = batch["input_ids"].shape[0]
        # print("current_action_mask: ", current_action_mask.sum(dim=1))
        # print("next_actions_mask: ", next_actions_mask.sum(dim=1))
        # print("current_action_mask | next_actions_mask: ", (current_action_mask | next_actions_mask).sum(dim=1))
...
def unwrap(model):
    """
    Unwraps a model from DistributedDataParallel (DDP) if it is wrapped.
...
def save_training_checkpoint(
    ...
    if distributed_state.is_main_process:
        ...
        torch.save(train_state, checkpoint_dir / f"train_state--{checkpoint_name_suffix}")
    ...
    if distributed_state.is_main_process:
        ...
        if not cfg.use_lora:
            unwrap(vla).save_pretrained(checkpoint_dir) # directly save checkpoint without lora
        else:
        ...
        if cfg.use_film:
            ...
            torch.save(
                unwrap(vla).vision_backbone.state_dict(), checkpoint_dir / f"vision_backbone--{checkpoint_name_suffix}"
            )
    ...
    if cfg.use_lora and cfg.merge_lora_during_training and log_step == cfg.max_steps:
        ...
        device_id = next(unwrap(vla).parameters()).device
        print("device id: ", device_id)
        unwrap(vla).to("cpu")
        torch.cuda.empty_cache()

        if cfg.use_minivlm or cfg.use_prismatic_vlm:
            if cfg.use_minivlm:
            ...
            # saving only the adapter (unwrap(vla).save_pretrained(adapter_dir)) will not include updated action_queries
            if 'module.base_model.model.action_queries.weight' in vla.state_dict():
...
def finetune(cfg: FinetuneConfig) -> None:
    ...
    if distributed_state.is_main_process:
        wandb.init(project=cfg.wandb_project, name=f"ft+{run_id}") # , mode="offline" , resume="allow"
    ...
    if cfg.lpft_path is not None:
    ...
    elif cfg.use_minivlm or cfg.use_prismatic_vlm:
        # read hf_token from ".hf_token" file
        hf_token = ''
        ...
        if cfg.resume:
            if cfg.use_lora:
                step = cfg.resume_vla_path.split('--')[-1].replace('_chkpt', '')
                checkpoint_path = os.path.join(cfg.resume_vla_path, f"action_queries--{step}_checkpoint.pt")
    ...
    if cfg.use_lora:
        if cfg.resume:
            ...
            #     vla,
            #     os.path.join(cfg.resume_vla_path, "lora_adapter"),
            #     is_trainable=True,          # <-- important
            # )
            # vla = vla.to(device_id)
            # Do NOT merge if you want to keep training:
            # vla = vla.merge_and_unload()  # <-- leave this commented out
            ...
            adapter_dir = os.path.join(cfg.resume_vla_path, "lora_adapter")
            
            from safetensors.torch import load_file
            peft_sd = load_file(os.path.join(adapter_dir, "adapter_model.safetensors"), device=device_id)

            # This will fill matching Linear LoRA layers; any Conv2d LoRA weights in the file are simply unused.
            set_peft_model_state_dict(vla, peft_sd)
        else:
            lora_config = LoraConfig(
                r=cfg.lora_rank,
                lora_alpha= 2 * cfg.lora_rank,
    ...
    if cfg.freeze_vlm:
        ...
        if cfg.unfreeze_early_language:
            # Unfreeze early LLM layers
            for i in range(len(vla.language_model.model.layers) - 1):
                early_layer = vla.language_model.model.layers[i]
                print(f"Unfreezing early language model layer: {early_layer}")
                early_layer.requires_grad_(True)
            print(f"Unfreezing the early layers of the language model.")
    ...
    if cfg.unfreeze_dino:
        for name, param in vla.named_parameters():
            if "action_queries" in name:
                ...
                param.requires_grad = True
        print("Unfreezing all parameters of the DINO featurizer.")
    ...
    if cfg.use_film:
        ...
        if cfg.resume:
            state_dict = load_checkpoint("vision_backbone", cfg.resume_vla_path, cfg.resume_step)
    ...
    if cfg.use_proprio:
        proprio_projector = init_module(
            ...
            cfg,
            device_id,
            {"llm_dim": unwrap(vla).llm_dim, "proprio_dim": PROPRIO_DIM},
            to_bf16=True,
        )
    ...
    if cfg.optimizer == "AdamW":
        ...
        if cfg.weight_decay_scheduler == "constant":
            ...
            if getattr(cfg, "robust_ft_early_llm_layers", False):
                if (
                    hasattr(unwrap(vla), "language_model") and
                    hasattr(unwrap(vla).language_model, "model") and
                    hasattr(unwrap(vla).language_model.model, "layers")
                ):
                    try:
                        layers_ref = unwrap(vla).language_model.model.layers
            ...
            for name, param in unwrap(vla).named_parameters():
            ...
            root = unwrap(vla) if hasattr(vla, "module") else vla
        ...
        elif cfg.weight_decay_scheduler == "layerwise_decay_cosine":
            import re
            from collections import OrderedDict, defaultdict
            import math
            ...
            for name, p, layer_key in params_buffer:
                ...
                # weight decay with cosine schedule
                wd_value = 0.5 * (1.0 + math.cos(math.pi * float(layer_idx) / float(total_layers)))
                wd_value = max(0.0, min(1.0, wd_value))  # clamp safety
        ...
        if cfg.weight_decay_scheduler == "layerwise_decay" or cfg.weight_decay_scheduler == "layerwise_decay_cosine":
    ...
    # if cfg.resume:
    #     try:
    #         train_state = load_checkpoint("train_state", cfg.resume_vla_path, cfg.resume_step)
    #         optimizer.load_state_dict(train_state["optimizer"])
    #         scheduler.load_state_dict(train_state["scheduler"])
            
    #         # (Important) Make optimizer state tensors contiguous in memory
    #         # for state in optimizer.state.values():
    #         #     for k, v in state.items():
    #         #         if torch.is_tensor(v) and not v.is_contiguous():
    #         #             state[k] = v.contiguous()
    #     except Exception as e:
    #         print(f"Error resuming optimizer and scheduler: {e}")

    # Create Action Tokenizer
    if cfg.use_vq_action_tokenizer:
        if cfg.use_minivlm:
            action_tokenizer = ACTION_TOKENIZERS["libero_vq_extra_action_tokenizer"](processor.tokenizer)
        else:
            action_tokenizer = ACTION_TOKENIZERS["libero_vq_action_tokenizer"](processor.tokenizer)
        assert action_tokenizer.required_future_horizon + 1 == NUM_ACTIONS_CHUNK, f"Expected {NUM_ACTIONS_CHUNK} action chunks for Libero VQ tokenizer, but got {action_tokenizer.required_future_horizon + 1}."
        assert cfg.use_l1_regression == False, "L1 regression head is not compatible with VQ action tokenizer."
        # assert action_tokenizer.action_token_begin_idx == ACTION_TOKEN_BEGIN_IDX, f"Expected Libero VQ action tokenizer to have begin idx {ACTION_TOKEN_BEGIN_IDX + 256}, but got {action_tokenizer.action_token_begin_idx}."
    else:
        if cfg.use_minivlm:
            action_tokenizer = ACTION_TOKENIZERS["extra_action_tokenizer"](processor.tokenizer)
        else:
            action_tokenizer = ACTION_TOKENIZERS["action_tokenizer"](processor.tokenizer)
        # assert action_tokenizer.action_token_begin_idx == ACTION_TOKEN_BEGIN_IDX, f"Expected Action tokenizer to have begin idx {ACTION_TOKEN_BEGIN_IDX}, but got {action_tokenizer.action_token_begin_idx}."
    ...
    train_dataset = RLDSDataset(
        ...
        cfg.dataset_name,
        batch_transform,
        resize_resolution=tuple(unwrap(vla).config.image_sizes),
    ...
    def get_mean_squared_grad_norm(module):
        total_sq = 0.0
        count = 0
        for p in module.parameters():
            if p.grad is not None:
    ...
    # Open CSV file for writing (do this once, e.g., before training loop)
    # csv_file = open('grad_norms.csv', 'w', newline='')
    # csv_writer = csv.writer(csv_file)
    # csv_file_2 = open('mean_squared_grad_norms.csv', 'w', newline='')
    # csv_writer_2 = csv.writer(csv_file_2)
    # modules = {
    #     "vision_backbone.featurizer": unwrap(vla).vision_backbone.featurizer,
    #     "vision_backbone.fused_featurizer": unwrap(vla).vision_backbone.fused_featurizer,
    #     "projector": unwrap(vla).projector,
    #     "language_model.model": unwrap(vla).language_model.model,
    #     "language_model.lm_head": unwrap(vla).language_model.lm_head,
    # }
    # csv_writer.writerow(['step'] + list(modules.keys()))
    # csv_writer_2.writerow(['step'] + list(modules.keys()))
    ...
    with tqdm.tqdm(total=cfg.max_steps, initial=start_step, leave=False) as progress:
        ...
        for batch_idx, batch in enumerate(dataloader):
            ...
            # grad_norms = []
            # mean_squared_grad_norms = []
            # for name, module in modules.items():
            #     grad_norm = get_grad_norm(module)
            #     mean_squared_grad_norm = get_mean_squared_grad_norm(module)
            #     grad_norms.append(grad_norm)
            #     mean_squared_grad_norms.append(mean_squared_grad_norm)
            # # Write grad norms to CSV
            # if distributed_state.is_main_process and log_step % cfg.wandb_log_freq == 0:
            #     csv_writer.writerow([log_step] + grad_norms)
            #     csv_writer_2.writerow([log_step] + mean_squared_grad_norms)
            #     csv_file.flush()
            #     csv_file_2.flush()

#             # ---- Layerwise grad norm plot (after backward, before optimizer.step/zero_grad) ----
#             if distributed_state.is_main_process and (log_step % cfg.wandb_log_freq == 0):
#                 # 1) Layerwise RAW grad norm (L2 over all params in layer)
#                 lw_raw = compute_layerwise_grad_norm(unwrap(vla))

#                 # 2) Layerwise RMS grad norm (size-normalized)
#                 lw_rms = compute_layerwise_rms_grad_norm(unwrap(vla))
...
#                 ax.bar(list(lw_raw.keys()), list(lw_raw.values()))
#                 ax.set_title(f"Layerwise Grad Norm (L2) @ step {log_step}")
#                 ax.set_xlabel("Layer")
#                 ax.set_ylabel("L2 grad norm")
#                 ax.tick_params(axis="x", labelrotation=45)
#                 fig.tight_layout()

#                 plot_path_raw = run_dir / f"layerwise_gradnorm_L2_step_{log_step}.png"
#                 fig.savefig(plot_path_raw, dpi=300)
#                 plt.close(fig)

#                 wandb.log({"GradNorm/LayerwiseL2_Image": wandb.Image(str(plot_path_raw))}, step=log_step)

#                 # also log lw_raw in a csv file for each step
#                 if not os.path.exists(run_dir / 'layerwise_gradnorm_L2.csv'):
#                     with open(run_dir / 'layerwise_gradnorm_L2.csv', 'w', newline='') as csvfile:
#                         csv_writer = csv.writer(csvfile)
#                         header = ['step'] + list(lw_raw.keys())
#                         csv_writer.writerow(header)
#                 with open(run_dir / 'layerwise_gradnorm_L2.csv', 'a', newline='') as csvfile:
#                     csv_writer = csv.writer(csvfile)
#                     row = [log_step] + [lw_raw[k] for k in lw_raw.keys()]
#                     csv_writer.writerow(row)

#                 # ---- Plot RMS ----
#                 fig = plt.figure(figsize=(12,6))
#                 ax = fig.add_subplot(111)
#                 ax.bar(list(lw_rms.keys()), list(lw_rms.values()))
#                 ax.set_title(f"Layerwise RMS Grad Norm @ step {log_step}")
#                 ax.set_xlabel("Layer")
#                 ax.set_ylabel("RMS grad norm")
#                 ax.tick_params(axis="x", labelrotation=45)
#                 fig.tight_layout()

#                 plot_path_rms = run_dir / f"layerwise_gradnorm_RMS_step_{log_step}.png"
#                 fig.savefig(plot_path_rms, dpi=300)
#                 plt.close(fig)

#                 wandb.log({"GradNorm/LayerwiseRMS_Image": wandb.Image(str(plot_path_rms))}, step=log_step)

#                 # log lw_rms in a csv file for each step
#                 if not os.path.exists(run_dir / 'layerwise_gradnorm_RMS.csv'):
#                     with open(run_dir / 'layerwise_gradnorm_RMS.csv', 'w', newline='') as csvfile:
#                         csv_writer = csv.writer(csvfile)
#                         header = ['step'] + list(lw_rms.keys())
#                         csv_writer.writerow(header)
#                 with open(run_dir / 'layerwise_gradnorm_RMS.csv', 'a', newline='') as csvfile:
#                     csv_writer = csv.writer(csvfile)
#                     row = [log_step] + [lw_rms[k] for k in lw_rms.keys()]
#                     csv_writer.writerow(row)
