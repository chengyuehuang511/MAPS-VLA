...
class ParallelLiberoEvaluator:
    ...
    def evaluate_episodes(self, gpu, task_ids_and_episodes, show_detail, summaries):
        try:
            model, action_head, action_tokenizer, proprio_projector, noisy_action_projector, processor = self._build_policy(gpu)
            reset_logging()
            self._build_logger(mode='a')

            for task_id, episode in task_ids_and_episodes:
                self.logger.info(f"GPU {gpu}: task {task_id} episode {episode}")
                summary = self.evaluate_single(model, processor, action_head, action_tokenizer, proprio_projector, noisy_action_projector, task_id, episode, show_detail)
    ...
    def evaluate_single(self, model, processor, action_head, action_tokenizer, proprio_projector, noisy_action_projector, task_id, episode, show_detail):
        ...
        # Set initial state if provided # 2
        if not self.cfg.task_suite_name == 'libero_object':
    ...
    def _build_policy(self, gpu):
        ...
        if self.cfg.use_vq_action_tokenizer:
            action_tokenizer: ActionTokenizer = ACTION_TOKENIZERS["libero_vq_extra_action_tokenizer"](processor.tokenizer)
            from prismatic.vla.constants import NUM_ACTIONS_CHUNK
            ...
            assert self.cfg.use_l1_regression == False, "L1 regression head is not compatible with VQ action tokenizer."
        else:
            action_tokenizer = ActionTokenizer(processor.tokenizer)

        return model, action_head, action_tokenizer, proprio_projector, noisy_action_projector, processor
...
def main(args):
    cfg = GenerateConfig(
        ...
        use_vq_action_tokenizer=args.use_vq_action_tokenizer,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--use-vq-action-tokenizer', action='store_true')
